...
// Prófaðu fallið:
const formattedPrice = formatPrice(123000);
console.log(formattedPrice); // Ætti að skrifa út `123.000 kr.`
...
function formatPrice(price) {
  // Hér þurfum við að setja inn útfærslu fyrir formatPrice. Ég hef ekki séð útfærslu þína hér, en geri ráð fyrir því að þú hafir eða vilt búa til eina.
...
function formatProduct(product, quantity = undefined) {
  if (quantity) {
    const total = product.price * quantity; // Calculate the total price here
    return `${product.title} — ${quantity}x${formatPrice(product.price)} samtals ${formatPrice(total)}`;
  }
  return `${product.title} — ${formatPrice(product.price)}`;
}
...
function cartInfo(cart) {
  ...
  for (const cartLine of shoppingCart.lines) { // Use cart.lines instead of cart.items
    const productTotal = cartLine.product.price * cartLine.quantity;
    totalPrice += productTotal;
    info += formatProduct(cartLine.product, cartLine.quantity) + '\n';
  }

  info += 'Samtals: ' + formatPrice(totalPrice);
...
function showProducts() {
  for (const product of products) {
    console.log(`#${product.id} ${product.title} — ${product.description} — ${formatPrice(product.price)}`);
  }
}
...
function addProductToCart() {
  const productId = parseInt(prompt('Sláðu inn auðkenni vöru sem þú vilt bæta við körfu:'), 10);
  ...
  const products = productsList.find(p => p.id === productId);
  ...
  const quantity = parseInt(prompt('Hversu mörg eintök af þessari vöru viltu bæta í körfu?'), 10);
  ...
  const cartLine = shoppingCart.lines.find(line => line.product.id === productId);
 ...
 */
  ...
  const cart = {
    items: [],  // Listi af vörum og fjölda þeirra
  };
  
  function formatPrice(price) {
    // Athugaðu að þetta mun ekki virka í Chrome vegna vandamála með Íslensku þar, 
    // en ætti að virka í flestum öðrum vafra.
    return new Intl.NumberFormat('is-IS').format(price) + ' kr.';
  }
  ...
  function cartInfo(cart) {
    ...
    for (const cartLine of shoppingCart.lines) { // Change cart.items to cart.lines
      const productTotal = cartLine.product.price * cartLine.quantity;
      totalPrice += productTotal;
      info += formatProduct(cartLine.product, cartLine.quantity) + '\n';
    }
  ...
  function showCart() {
    if (shoppingCart.lines.length === 0) {
      console.log('Karfan er tóm.');
      return;
    }
  
    // Display cart information here using the cartInfo function
    const cartInformation = cartInfo(shoppingCart);
...
function checkout() {
  // Athugum fyrst hvort körfan sé tóm
  if (shoppingCart.lines.length === 0) {
    console.log("Karfan er tóm.");
    return;
  }

  // Biðjum notandann um nafn og heimilisfang
  const name = prompt("Vinsamlegast sláðu inn þitt nafn:");
  const address = prompt("Vinsamlegast sláðu inn þitt heimilisfang:");

  // Athuga hvort nafn eða heimilisfang sé tómt
  if (!name || !address) {
    console.log("Vinsamlegast fylltu út bæði nafn og heimilisfang.");
    return;
  }

  // Birta kvittun í console
  console.log(`Pöntun móttekin ${name}.`);
  console.log(`Vörur verða sendar á ${address}.\n`);

  // Birta vörulista úr körfu
  for (const item of shoppingCart.lines) {
    console.log(formatProduct(item.product, item.quantity));
  }

  // Birta heildarverð körfunnar
  const total = shoppingCart.lines.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
